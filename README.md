...
- `NEXT_PUBLIC_LIVEAVATAR_SANDBOX`: set to `true` in development to use sandbox sessions (no credits consumed). **Sandbox only supports the default public avatar (June HR)** — the LiveAvatar API rejects any other `avatar_id` in sandbox mode, so a character's chosen avatar is only used when this is `false` (real sessions that consume credits). Live avatar streams are capped at **60 seconds** per session.
